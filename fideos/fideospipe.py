...
if (pre_process == 1):
	...
	MasterBias = fits.open(dirout+'MasterBias.fits')
	...
	if just_bias == False:
	    ...
	    hdu1 = fits.PrimaryHDU( MasterFlat )
	    ...
	    hdu = fits.PrimaryHDU( MasterFlat_co )
        ...
        mfdate = str(datetime.datetime.now()).replace(' ','T')
        hdu = fits.PrimaryHDU( MasterFlat_ob )
        hdu = GLOBALutils.update_header(hdu,'BDATE',MasterBias[0].header['DATE'])
        hdu = GLOBALutils.update_header(hdu,'DATE',mfdate)
...
MasterBias = fits.open(dirout+'MasterBias.fits')
ronoise = MasterBias[0].header['RON']
MasterFlat = fits.open(dirout+'MasterFlat.fits')
MasterFlat_co = fits.open(dirout+'MasterFlat_co.fits')
MasterFlat_ob = fits.open(dirout+'MasterFlat_ob.fits')
...
if ( os.access(P_ob_fits,os.F_OK) == False ) or ( os.access(S_flat_ob_fits,os.F_OK) == False ) or \
    ...
    hdu = fits.PrimaryHDU( P_ob )
    hdu.writeto( P_ob_fits )
    hdu = fits.PrimaryHDU( P_co )
    ...
    hdu = fits.PrimaryHDU( S_flat_ob_simple )
    ...
    hdu = fits.PrimaryHDU( S_flat_co_simple )
    hdu.writeto( S_flat_co_simple_fits )

print "Extracted flat comparison spectra found, loading..."
P_ob       = fits.getdata( P_ob_fits )
S_flat_ob  = fits.open( S_flat_ob_fits )
S_flat_ob_simple = fits.getdata( S_flat_ob_simple_fits )
P_co       = fits.getdata( P_co_fits )
S_flat_co  = fits.open( S_flat_co_fits )
S_flat_co_simple = fits.getdata( S_flat_co_simple_fits )
...
thar_times = []
print "================================================================================================================"
print "===================                   Step 3: ThAr lamps: extraction                        ===================="
print "================================================================================================================"
for fsim in thars:
    hthar = fits.open( fsim )
    dthar = fits.getdata( fsim ) - MasterBias[0].data
    ...
    if ( os.access(thar_fits_ob_simple,os.F_OK) == False ) or ( os.access(thar_fits_co_simple,os.F_OK) == False ) or (force_thar_extract) or (just_thar):
        print "No previous extraction for ThAr file", fsim[fsim.rfind('/')+1:], "extracting..."
        ...
        hdu = fits.PrimaryHDU( thar_Ss_ob )
        hdu = GLOBALutils.update_header(hdu,'BDATE',MasterBias[0].header['DATE'])
        ...
        hdu = fits.PrimaryHDU( thar_Ss_co )
        hdu = GLOBALutils.update_header(hdu,'BDATE',MasterBias[0].header['DATE'])
...
tharsused=thars[0]
print 'Using image:', tharsused[tharsused.rfind('/')+1:]
print "================================================================================================================"
print "========================           Step 6: Computing wavelength calibration            ========================="
print "================================================================================================================"
...
if os.access(dirout+'order_id.pkl', os.F_OK) == False or force_thar_id:
    ...
    if not just_comp:
        ...
        thar_ob = fits.getdata(thar_fits_ob)[:,1,:]
    ...
    thar_co = fits.getdata(thar_fits_co)[:,1,:]
...
for fsim in thars:
    ...
    if ( os.access(wavsol_pkl,os.F_OK) == False ) or (force_thar_wavcal):
        print "Working on initial ThAr file", fsim[fsim.rfind('/')+1:]
        hthar = fits.open( fsim )
        mjd, mjd0 = fideosutils.mjd_fromheader( hthar )
        thar_fits_ob = dirout + fsim.split('/')[-1][:-4]+'spec.ob.fits.S'
        thar_S_ob = fits.open( thar_fits_ob )
        thar_fits_co = dirout + fsim.split('/')[-1][:-4]+'spec.co.fits.S'
        thar_S_co = fits.open( thar_fits_co )
        ...
        hdu = fits.PrimaryHDU(thar_out_co)
        ...
        thar_out = fits.getdata(thar_spec_ob)
        if fsim == thars[0]:
            ref_pix = thar_out[0,25,1024]
if DoFull:
	...
	for fsim in thars:
		...
		print "Wavelength solution including reddest orders of", fsim[fsim.rfind('/')+1:]
		hthar = fits.open( fsim )
		thar_fits_ob = dirout + fsim.split('/')[-1][:-4]+'spec.ob.fits.S'
		thar_S_ob = fits.open( thar_fits_ob )
...
print "================================================================================================================"
print "============================            Step 7: Instrumental drift            =================================="
print "================================================================================================================"
...
if len(thars)>0:
	if force_drifts == True or os.path.isfile(drift_names_file) == False or os.path.isfile(drift_file) == False:
	    for refthid in range(len(thars)):
		    ...
		    thar_S_co = fits.getdata( thar_fits_co )

		    print 'Computing instrumental drifts for ThAr + ThAr imeges...'

		    for i in range(len(thars)):
			texp = fits.getheader(thars[i])['EXPTIME']
			pkl  = dirout + thars[i].split('/')[-1][:-4]+'wavsolpars.pkl'
			wsol = pickle.load(open(pkl,'r'))
			mjd, mjd0 = fideosutils.mjd_fromheader( fits.open(thars[i]) )
...
tharsbest=thars[best_id]
print 'Reference wavelength image will be:', tharsbest[tharsbest.rfind('/')+1:]  #thars[best_id]
...
print "Unpickling wavelength solution from", pkl_wsol[pkl_wsol.rfind('/')+1:], " ..."
wsol_dict = pickle.load(open(pkl_wsol,'r'))

#print thars_co
print 'Computing instrumental drifts for ThAr + Dark imeges...'
print 'Reference wavelength image will be:', tharsbest[tharsbest.rfind('/')+1:]  #thars[best_id]
pkl_wsol2 = dirout + thars[best_id].split('/')[-1][:-4]+'wavsolpars.pkl'
print "Unpickling wavelength solution from", pkl_wsol2[pkl_wsol2.rfind('/')+1:], " ..."
wsol_dict2 = pickle.load(open(pkl_wsol2,'r'))
sci_all = np.hstack((sim_sci, dar_sci))

if len(thars_co)>0:
	thar_fits_co2 = dirout + thars_co[int(.5*len(thars_co))].split('/')[-1][:-4]+'spec.co.fits.S'
	thar_S_co2 = fits.getdata( thar_fits_co2 )

	for i in range(len(thars_co)):
		    texp = fits.getheader(thars_co[i])['EXPTIME']
		    pkl  = dirout + thars_co[i].split('/')[-1][:-4]+'wavsolpars.pkl'
		    wsol = pickle.load(open(pkl,'r'))
		    hthar = fits.open( thars_co[i] )
		    mjd, mjd0 = fideosutils.mjd_fromheader( hthar )
		    p_shift_co, pix_centers, orders, wavelengths, I, rms_ms_co, residuals_co  = \
...
print "================================================================================================================"
print "============                     Step 8: Science - extraction and analysis                   ==================="
print "================================================================================================================"
...
print 'The following targets will be processed:'
...
for fsim in new_list:
    try:
	    print "--> Working on image: ", fsim[fsim.rindex('/')+1:]
	    hd = fits.getheader(fsim)
		...
		#if args.reffile != 'default':
		#	print "\t\tUsing coordinates from reffile"
		#	ra2,dec2 = GLOBALutils.getcoords(obname,mjd,filen=reffile)
		#	print "Coordinate",ra2,dec2
		#	if ra2 !=0 and dec2 != 0:
		#		ra = ra2
		#		dec = dec2
		#	else:
	    ...
	    #else:
		#	print "\t\tSearching coordinates in Simbad"
	    ...
	    print "\tBarycentric velocity:", bcvel_baryc, 'km/s'
	    ...
	    print '\tRadial Velocity of sacttered moonlight:',refvel,'km/s'

	    if os.access(dirout+'Dark_'+str(exptime)+'.fits',os.F_OK):
		DARK = fits.getdata(dirout+'Dark_'+str(exptime)+'.fits')
	    else:
		DARK = np.zeros(MasterBias[0].data.shape)

	    hdat  = fits.open( fsim )
	    data  = fits.getdata( fsim )
		...
		hdu = fits.PrimaryHDU(bac)
		hdu.writeto(bac_fits)
	    else:
		bac = fits.getdata(bac_fits)
		...
		( force_sci_extract ):
		print "\tNo previous extraction for science file", fsim[fsim.rfind('/')+1:], "extracting..."
		...
		hdu = fits.PrimaryHDU( sci_S_ob )
		hdu.writeto( sci_fits_ob )
		hdu = fits.PrimaryHDU( sci_Ss_ob )
	    ...
	    if fsim in sim_sci:
		    ...
		    hdu = fits.PrimaryHDU( sci_S_co )
		    hdu.writeto( sci_fits_co )
		    hdu = fits.PrimaryHDU( sci_Ss_co )
		    hdu.writeto( sci_fits_co_simple )
	    else:
			print fsim, "has already been extracted, reading in product fits files..."
			sci_S_ob  = fits.getdata( sci_fits_ob )
			sci_Ss_ob = fits.getdata( sci_fits_ob_simple )
			bac_ob = fits.getdata( bac_fits_ob_simple )

	    p_shift = 0.

	    if fsim in sim_sci:
		sci_S_co  = fits.getdata( sci_fits_co )
		sci_Ss_co = fits.getdata( sci_fits_co_simple )
	    ...
	    print "\tWavelngth calibration--> done!"
	    ...
	    hdu = GLOBALutils.update_header(hdu,'OBJNAME', obname)
	    hdu = GLOBALutils.update_header(hdu,'RA',ra)
	    hdu = GLOBALutils.update_header(hdu,'DEC',dec)
	    ...
	    hdu = GLOBALutils.update_header(hdu,'AIRMASS',np.around(airmass.value,3))
	    hdu = GLOBALutils.update_header(hdu,'BDATE',MasterBias[0].header['DATE'])
	    ...
	    if DoFull:
		    ...
		    hdufull = fits.PrimaryHDU( spec_full )
		    ...
		    hdufull = GLOBALutils.update_header(hdufull,'RA',ra)
		    hdufull = GLOBALutils.update_header(hdufull,'DEC',dec)
		...
		print '\t\tRV = '+str(RV)+' +- '+str(RVerr2),'km/s'
		print '\t\tBS = '+str(BS)+' +- '+str(BSerr),'km/s'
		...
		hdu = GLOBALutils.update_header(hdu,'INSTRUME', 'FIDEOS')
		hdu = GLOBALutils.update_header(hdu,'TELESCOP', 'LSO1')
		...
		hdu = GLOBALutils.update_header(hdu,'CCDRON',ronoise)
		hdu = GLOBALutils.update_header(hdu,'CCDGAIN',gain)
		...
		hd['EXPTIME'], SNR_5130_R, airmass, shift_pix, rv_drift_co, ccf_pdf[ccf_pdf.find('/')+1:])
	    ...
	    #    spec = fits.getdata( fout )
    except:
		print 'Problem with', fsim[fsim.rfind('/')+1:]
